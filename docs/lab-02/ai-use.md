...
| `feature/12-lab2-e2e-and-artifacts` | A final compliance pass against the labsheet found the E2E/Playwright layer and `artifacts/lab-02/screenshots/` were both still missing, and that `specification.md`'s FR-03/BR-21/AC-07 described a create-time attachment upload that was never built. Added Playwright (`playwright.config.ts`, `e2e/lab-02/requester-ticket-flow.spec.ts` — select Requester → create Ticket → find in My Tickets → open Ticket Detail, run against the real dev servers and database) and a real no-database unit test for `generateTicketNumber`. Rewrote FR-03/BR-21/AC-07 and the matching `tests.md` rows to document the as-built attachment-timing decision instead of the originally planned one. Captured real screenshots into `artifacts/lab-02/screenshots/`. Corrected two accuracy issues found while re-reading `reviewer.md` against the actual GitHub PR history via `gh api` (PR #22's second round was a plain "LGTM" comment, not a second formal Approve; the cross-package-import critique wasn't a verifiable GitHub review comment on PR #21/#22, so it's now described as review discussion rather than attributed to a specific formal review). |
...
| Ticket creation, less duplication | "Now let do Ticket creation... make sure there are as less duplicate code as possible. And no spaghetti code." | Extracted shared validation into a `ticket-rules.ts` module server and client both import, instead of duplicating length/enum constants. This is the module later review discussion flagged for the relative cross-package import risk — the DRY fix was right, the *mechanism* used to share it wasn't, and got corrected properly later. |
...
#21's ARIA/busy-state findings on GitHub, and the cross-package-import
critique raised in follow-up review discussion) was treated as something
to verify and act on, not rubber-stamp — the import critique in
particular was accepted and fixed with a real architectural change (an
npm workspace) rather than a superficial patch.
